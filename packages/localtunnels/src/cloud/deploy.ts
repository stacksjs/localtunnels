...
export function generateTunnelServerUserData(opts: {
  ...
  const lines = [
    ...
    'SERVERSCRIPT',
    '',
    '# Compile the stable server entry in Bun production mode',
    '/root/.bun/bin/bun build --production --compile server.ts --outfile localtunnel-server',
    'chmod 0755 localtunnel-server',
    '',
  ]
  ...
  if (enableSsl && domain) {
    lines.push(
      ...
      'ExecStart=/opt/localtunnel/localtunnel-server',
      'Restart=always',
      'RestartSec=30',
